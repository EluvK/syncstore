...
fn owner_basic_crud() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo_id = store.insert(namespace, "repo", &doc, user)?;
    ...
    let updated_item = store.update(namespace, "repo", &repo_id, &updated, user)?;
    assert!(updated_item.updated_at >= updated_item.created_at || updated_item.updated_at > item.created_at);
...
fn other_access_unauthorized() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repo_id = store.insert(namespace, "repo", &doc, user1)?;
...
fn insert_and_list_child_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let post_id1 = store.insert(namespace, "post", &post_doc1, user)?;
    let post_id2 = store.insert(namespace, "post", &post_doc2, user)?;
...
fn insert_and_list_owner_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (posts_page2, next_marker2) = store.list_by_owner(namespace, "post", next_marker1, 5, user1)?;
...
fn validate_child_parent_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let post_id = store.insert(namespace, "post", &valid_post_doc, user)?;
    ...
    let another_repo_id = store.insert(namespace, "repo", &another_repo_doc, user)?;
    ...
    let new_item = store.update(namespace, "post", &post_id, &updated_post_doc_valid, user)?;
    assert!(new_item.updated_at > new_item.created_at);
    assert!(new_item.id == post_id);
